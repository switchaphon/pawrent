...
-- 2026-06-01: corrected from recorded_at (nonexistent column) to measured_at; see lib/types/health.ts.
-- The original errored on apply, so this index may be missing on already-migrated DBs — re-run to create it.
CREATE INDEX IF NOT EXISTS idx_pet_weight_logs_pet_date ON pet_weight_logs(pet_id, measured_at DESC);
