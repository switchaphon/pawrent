...
describe("PhotoLightbox", () => {
  ...
  it("renders photo and counter when open", () => {
    render(<PhotoLightbox photos={mockPhotos} initialIndex={0} isOpen={true} onClose={vi.fn()} />);
  ...
  it("calls onClose when Escape key is pressed", () => {
    const onClose = vi.fn();
    render(<PhotoLightbox photos={mockPhotos} initialIndex={0} isOpen={true} onClose={onClose} />);
    fireEvent.keyDown(document, { key: "Escape" });
    expect(onClose).toHaveBeenCalled();
  });

  it("navigates with ArrowLeft and ArrowRight keys", () => {
    render(<PhotoLightbox photos={mockPhotos} initialIndex={1} isOpen={true} onClose={vi.fn()} />);
    expect(screen.getByText("2 / 3")).toBeInTheDocument();
  ...
  it("shows delete confirmation when delete button is clicked", async () => {
    ...
    const deleteBtn = screen
      .getAllByRole("button")
      .find((b) => b.querySelector(".lucide-trash-2") || b.textContent?.includes("Delete"));
    if (deleteBtn) {
  ...
  it("wraps around when navigating past last photo", async () => {
    render(<PhotoLightbox photos={mockPhotos} initialIndex={2} isOpen={true} onClose={vi.fn()} />);
    expect(screen.getByText("3 / 3")).toBeInTheDocument();
